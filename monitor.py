...
def require_env(key):
    ...
    if not value:
        raise OSError(f"Channels2MQTT: Required environment variable '{key}' is not set.")
    return value
...
def process_upcoming(client):
    ...
    upcoming = [build_upcoming_payload(job) for job in jobs if job.get("start_time", 0) >= now]

    payload = {"count": len(upcoming), "recordings": upcoming}
    ...
    while True:
        ...
        if recording:
            ...
            if current_id not in seen_recording_ids:
                ...
                log.info(
                    f"Channels2MQTT: New recording detected: {payload['title']} - {payload['episode']}"
                )
